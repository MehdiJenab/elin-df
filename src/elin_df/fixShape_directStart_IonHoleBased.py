...
class QuietStart(object):
	...
	def __init__(self):
		...
		self.nVx_Gq 			= [1000,1000] # since nvx should be large so searching beta can converge, nVx_Gq used for output
		self.iSpeciesElc 		= 0
		self.iSpeciesIon 		= 1
	...
	def write_Moments_DF_FixedSoliton(self, momentsElc,momentsIon,dfGqElc,dfGqIon,nameElc,nameIon,ix,dataFixed):
		...
		file_DF.close()
		# Safely remove trailing comma using utility (avoids seek/truncate on text mode)
		remove_trailing_comma(os.path.join(SCRIPT_DIR, 'QuietStart_Input.json'))
		# reopen and append the closing bracket for this entry
		file_DF = open(os.path.join(SCRIPT_DIR, 'QuietStart_Input.json'), 'a')
	...
	def insert_fixedSoliton(self):
		...
		# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		dataFixed = self.setFixedSoliton(n=4,a=a,l=l,x0=x0,vElc=v_soliton,vIon=v_holeIon)
	...
	def get_ElinDF_elc_ion(self,dataFixed):
		...
		while True:
			...
			df1dArr, vx1dArr = m.convert_2D_1D(df2dArr, vx2dArr)
			dfGqRefIon = m.get_interpolation(vx1dArr, self.vGqIon,df1dArr)
			...
			if isinstance(momentsElc, list) and isinstance(momentsRefIon, list):
				diff.append( momentsRefIon[0]-momentsElc[0])
				phiArr.append(phi0)
				self.write_Moments_DF_FixedSoliton(momentsElc,momentsRefIon,dfGqElc,dfGqRefIon,nameElc,nameIon,idx,dataFixed)
